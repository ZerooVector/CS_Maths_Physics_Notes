...
Z = XV
$$
意味着数据 $X$ 向着矩阵 $V$ 的各个列向量上投影，得到的 $Z$ 的一横行就意味着 $X$ 在各个 $v_i$ 上的坐标。那么我们现在将这一组规范正交基取为 SVD 分解中的 $V$ 矩阵的各列（由于 PCA 实际上就是向着 $V$ 的各个列上投影，所以这样做计算得到的就是每组数据的主成分），我们考虑将 $X$ 写成 SVD 分解的形式：
$$
