...
#### 对状态进行估价
利用一个状态未来可能得到的奖励的多少，我们可以对这个状态进行估价。估价函数就是从当前状态开始，收获的期望，也就是
$$
\begin{aligned}
...
$$
v_\pi(s) = E[R_s] +\gamma \sum_{\forall \ possible\ s'} P_{s\rightarrow s'}v_\pi (s')
$$
这里其实消去了一个求和号，因为对于行动的求和自然完成了。
将 $v$ 代入 $q$，那么：
$$
q_\pi(s,a) = R_s^a+\gamma \sum_{s'}\sum_{a}\pi(a|s')q_\pi(a|s')
$$
这其实也是本步行动的收益加上下一步行动的期望收益？ ==这里不太明确==
